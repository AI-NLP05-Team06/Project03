"""KDIC Structured Hybrid + BGE Reranker + Parent-Child 평가기.

업무 라벨로 검색 범위를 제한하지 않는다. 제목·소제목·브레드크럼·본문을
BGE-M3로 임베딩한 Structured Dense 순위와 BM25-Nori Discard 순위를
가중 RRF로 결합하고 BAAI/bge-reranker-v2-m3로 후보 Child를 재정렬한다.
Reranker 이후 Child 검색 순위는 기존 검색 지표로 평가하고, 상위 Child를
같은 parent_doc_id의 인접 청크로 확장한 답변 컨텍스트는 별도
Parent-Child 지표로 평가한다. 처리 순서는 Hybrid -> Reranker -> Parent-Child다.
"""
...
from evaluate_hybrid_a_reranker import (
...
class ParentChildExpander:
    ...
    def expand(
        ...
        *,
        ...
        # max_parents는 '인접 청크를 확장할 Parent 수'만 제한한다.
        # 검색/Reranker가 선택한 seed Child 자체는 Parent 수와 관계없이 보존한다.
        ...
        def add_neighbor(chunk_id: str, seed_id: str, distance: int) -> None:
        ...
        # 먼저 모든 seed를 순위대로 보존한다. max_parents나 Parent별 제한 때문에
        # 4~5번째 seed가 사라지는 현상을 막는다.
        for seed_id in seeds:
            ...
            if not position_info:
                continue
            parent_id, _ = position_info
            if seed_id in expanded:
                continue
            chunk = self.by_id[seed_id]
            expanded.append(seed_id)
            evidence.append(
                {
                    "chunk_id": seed_id,
                    "parent_doc_id": parent_id,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--reranker-batch-size", type=int, default=DEFAULT_RERANKER_BATCH_SIZE
    )
    parser.add_argument(
...
def validate_args(args: argparse.Namespace) -> None:
    if args.candidate_k < 10 or args.rerank_k < 10 or args.final_k < 10:
        raise ValueError(
            "MRR@10과 MAP@10을 위해 candidate-k, rerank-k, final-k는 10 이상이어야 합니다."
        )
    ...
    if args.max_context_chunks < args.seed_child_k:
        raise ValueError("--max-context-chunks는 --seed-child-k 이상이어야 합니다.")
...
def main() -> int:
    ...
    retriever_name = (
        ...
        "+ BGE Reranker + Parent-Child Neighbor Expansion (no domain filter)"
    )
    run_config = {
        ...
        "rerank_candidate_k": args.rerank_k,
        ...
        "pipeline_order": ["structured_dense", "bm25_nori", "weighted_rrf", "reranker", "parent_child"],
    ...
    chunks_by_id = {
        clean_id(chunk.get("chunk_id")): chunk
        for chunk in chunks
        if clean_id(chunk.get("chunk_id"))
    }
    ...
    for number, question in enumerate(questions, start=1):
        ...
        hybrid_ids = [result["chunk_id"] for result in hybrid_pool]
        before_metrics = evaluate_ranking(
            hybrid_ids[: args.final_k],
        ...
        passages = [build_passage(chunks_by_id[result["chunk_id"]]) for result in hybrid_pool]
        ...
        reranked_results = sorted(
            [
                ...
                for rank, (result, score) in enumerate(
                    zip(hybrid_pool, reranker_scores), start=1
                )
            ],
            key=lambda result: (-result["reranker_score"], result["hybrid_rank"]),
        )
        rerank_latency_ms = (time.perf_counter() - rerank_started) * 1000
        final_results = reranked_results[: args.final_k]
        ...
        row = {
            ...
            "retrieved_scores": json.dumps(
                [round(result["reranker_score"], 8) for result in final_results]
            ),
            "hybrid_before_rerank_ids": json.dumps(
                hybrid_ids[: args.final_k], ensure_ascii=False
            ),
            "hybrid_candidate_ids": json.dumps(hybrid_ids, ensure_ascii=False),
            "hybrid_candidate_rrf_scores": json.dumps(
                [round(result["rrf_score"], 10) for result in hybrid_pool]
            ),
                ...
                [
                    {
                        ...
                        "reranker_score": round(result["reranker_score"], 8),
                    }
        ...
        print(
            ...
            f"BeforeHit@3={before_metrics['hit_at_3']:.0f} "
            f"AfterHit@3={child_metrics['hit_at_3']:.0f} "
    ...
    before_details = [
        {
            **row,
            **{
                key.removeprefix("before_"): value
    ...
    write_csv(args.output_dir / "summary_before_rerank_by_domain.csv", before_by_domain)

    summary = {
        **run_config,
        "metric_note": {
            "standard_metrics": "Reranker 이후, Parent 확장 전 Child 순위를 평가",
            "before_metrics": "before_ 접두사는 Reranker 이전 Hybrid Child 순위를 평가",
            "parent_child_metrics": "Reranker 상위 Child를 Parent 내 인접 청크로 확장한 답변 컨텍스트를 평가",
            "expanded_non_gold_ratio": "Gold 미표기 청크 비율이며 곧바로 무관 청크라는 뜻은 아님",
        },
        "before_rerank_overall": before_overall,
        "overall": overall,
        "before_rerank_by_domain": before_by_domain,
        "by_domain": by_domain,
