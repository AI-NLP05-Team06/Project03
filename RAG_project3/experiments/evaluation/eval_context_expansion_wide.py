# [Phase3 추가실험] "parent-child식" 컨텍스트확장: window=1(바로 옆 청크만)이
# 아니라 window=3(같은 문서 내 앞뒤 3개, 최대 7개 청크)으로 넓혀서 같은
# multi_chunk_required=Y 표본에 재검증한다. 전체 문서를 다 넣으면 프롬프트가
# 너무 커지므로(문서당 최대 64청크) "top-N"으로 절충했다.
# eval_context_expansion.py와 동일한 구조, window 값과 저장 파일명만 다르다.
import sys as _sys
...
SAMPLE_PER_DOMAIN = 2
RANDOM_SEED = 42
CONTEXT_WINDOW = 3  # window=1(기존 Phase3)보다 넓힌 "parent-child식" 설정
REFUSAL_TEXT = (
...
print(f"채점 대상 문항 수: {len(sampled_records)} (도메인 {len(by_domain)}개 x 최대 {SAMPLE_PER_DOMAIN}개, window={CONTEXT_WINDOW})")
...
detail_path = EXPERIMENTS_ROOT / "context_expansion_comparison_wide.csv"
rows = []
...
print(f"\n=== window={CONTEXT_WINDOW} 컨텍스트확장 전/후 fully_grounded 비율 ===")
print(f"채점 가능 문항: {len(judged)}/{len(df)}")
print(f"적용 전: {baseline_rate:.1%}")
print(f"적용 후: {expanded_rate:.1%}")

print(f"\n=== window={CONTEXT_WINDOW} completeness(빠진 내용 없음) 비율 ===")
