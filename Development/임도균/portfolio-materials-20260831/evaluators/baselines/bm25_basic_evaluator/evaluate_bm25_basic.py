"""KDIC BM25 기본 검색 일괄 평가기.

답변과 임베딩은 생성하지 않습니다. 기본 정규식 토큰화와 BM25 점수로
427개 청크를 검색하고 질문별 Top-10을 Gold 청크와 비교합니다.
"""
...
TOKEN_PATTERN = re.compile(r"[가-힣]+|[a-z]+|\d+(?:[.,]\d+)*")


def basic_tokenize(text: Any) -> list[str]:
    """형태소 분석 없이 한글·영문·숫자 덩어리를 추출하는 재현 가능한 기준선."""
    ...
    def __init__(
        ...
        *,
        ...
        self.term_frequencies = [
            Counter(basic_tokenize(chunk.get("content"))) for chunk in chunks
        ]
...
def main() -> int:
    ...
    dry_report = {
        ...
        "retriever": "BM25 Basic regex tokenizer",
        "dataset": str(args.dataset.resolve()),
        "kdic_zip": str(args.kdic_zip.resolve()),
        "question_count": len(questions),
        "chunk_count": len(chunks),
        "tokenizer": "unicode_nfkc_lowercase_korean_english_number_regex",
        "k1": args.k1,
        "b": args.b,
    ...
    if args.dry_run:
        print("Dry-run 완료: BM25 검색은 실행하지 않았습니다.")
        return 0

    print(f"BM25 인덱스 생성: k1={args.k1}, b={args.b}")
    index = BM25Index(
        chunks=chunks,
        k1=args.k1,
        b=args.b,
    ...
    for number, question in enumerate(questions, start=1):
        ...
        details.append({
            ...
            "query_token_count": len(basic_tokenize(question.question)),
        })
        print(
